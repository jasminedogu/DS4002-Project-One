...
onehot_train <- combined_df[data_train, 29:45] #Should contain 70% of data points
onehot_val <- combined_df[data_val, 29:45]
onehot_val <- combined_df[data_test, 29:45]
...
splitting_data_test <- sample(1:nrow(df),
               round(0.15 * nrow(df), 0), 
               replace = FALSE)
splitting_data_validation <- sample(1:nrow(df),
               round(0.15 * nrow(df), 0), 
               replace = FALSE)
...
# Creating a 70/30 split
splitting_data_onehot <- sample(1:nrow(df_onehot),
               round(0.7 * nrow(df_onehot), 0), 
               replace = FALSE)

splitting_data_test_onehot <- sample(1:nrow(df_onehot),
               round(0.15 * nrow(df_onehot), 0), 
               replace = FALSE)

#Creating the train and test data
svm_train_onehot <- df_onehot[splitting_data_onehot, ] #Should contain 70% of data points
svm_test_onehot <- df_onehot[splitting_data_test_onehot, ] #Should contain 15% of data points

#Checking to ensure steps above were done correctly
size_of_training <- nrow(svm_train_onehot)
size_of_total <- nrow(df_onehot)
size_of_test <- nrow(svm_test_onehot)
...
## Splitting the Training, Validating, and Testing Sets
```{r}
splitting_data_rf <- sample(1:nrow(df),
               round(0.7 * nrow(df), 0), 
               replace = FALSE)

splitting_data_rf_val <- sample(1:nrow(df),
               round(0.15 * nrow(df), 0), 
               replace = FALSE)

splitting_data_rf_test <- sample(1:nrow(df),
               round(0.15 * nrow(df), 0), 
               replace = FALSE)
```
This is creatinng the training, validating, and testing set for the original data prior to feature selection. Our group has chosen to use the 70-15-15 ratio here as well. 
...
mytry_tune(df)
```
Using the mtry_tune function, we find that we should run our initial RF model using mtry = 5.196152, meaning that at each split, five variables will be randomly sampled as candidates at each split.
