...
onehot_test <- combined_df[data_test, 29:46]
...
```
With regard to our key metric, the F1 Score depreciated t0 0.1817, which is awful. Now, we will set to tuning the hyperparameters, using the original data that did not have a reduced feature space. 
...
```{r}
f1score_radial_onehot = 2*0.9374 *(539/(539+158))/(0.9374  + (539/(158+539)) )
f1score_radial_onehot

```
The F1 Score unfortunately, increased the F1 Score to 0.8
