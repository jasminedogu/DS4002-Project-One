...
**Goal**: Through this project, our group hopes to utilize both the **Support Vector Machine** (SVM) and **Random Forest Classifier** models to predict whether a person is or is not going to get the H1N1 vaccine. These two algorithms were selected for the reasons that are provided below. We will first begin by explaining some background information on the H1N1 virus, the dataset, and the two machine learning algorithms. From there, we will proceed with the model creation and the parameter tuning. Lastly, we will draw conclusions from our findings and discuss future applications. 
...
h1n1_correlation = ggcorr(df, method = c("everything", "pearson"), title = "H1N1 Factors Correlogram")
h1n1_correlation
```
...
```{r}
pre_1 <- ggplot(df, aes(x=h1n1_concern)) +
...
```{r}
pre_2 <- ggplot(df, aes(x=h1n1_knowledge)) + 
...
```{r}
beh1 <- ggplot(df, aes(x=behavioral_wash_hands)) + 
...
```{r}
beh2 <- ggplot(df, aes(x=behavioral_large_gatherings)) + 
...
```{r}
opi1 <- ggplot(df, aes(x=opinion_h1n1_risk)) + 
...
```{r}
opi2 <- ggplot(df, aes(x=opinion_h1n1_sick_from_vacc)) + 
...
```{r}
dem1 <- ggplot(df, aes(x=age_group)) + 
...
```{r}
dem2 <- ggplot(df, aes(x=education)) + 
...
```{r}
dem3 <- ggplot(df, aes(x=employment_status)) + 
...
```{r}
hc1 <- ggplot(df, aes(x=doctor_recc_h1n1)) + 
...
```{r}
hc2 <- ggplot(df, aes(x=chronic_med_condition)) + 
