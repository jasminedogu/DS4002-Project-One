...
```
The Aggregated Random Forest Model Results are shown above. The best performing model in terms of F1 Score was the original polynomial kernel. We want to select the tuned random forest for further exploration and use, due to the decrasedd computing burden necessary. This is because the tuned model utilized 188 trees, as opposed to the 200 trees utilized in the original random forest model, which only performed slightly better (F1 Score 0.0015 better). 
