...
```{r, include=FALSE}
df_features <- read.csv("Datasets/training_set_features.csv")
df_label <- read.csv("Datasets/training_set_labels.csv")
...
```
The Aggregated Random Forest Model Results are shown above. The best performing model in terms of F1 Score was the original polynomial kernel. We want to select the tuned random forest for further exploration and use, due to the decrased computing burden necessary. This is because the tuned model utilized 188 trees, as opposed to the 200 trees utilized in the original random forest model, which only performed slightly better (F1 Score 0.0015 better). The tuned random forest, utilizing the original data, had an F1 Score of 0.8655. Noticeably, the feature reduced models performed relatively poorly. Tuning the feature reduced random forest had little to no effect across the board. 
...
We utilized a Wilcoxon Rank Sum Test as it is a non-parametric test that does not assume the sample has a known distribution, which works for our data as we would only be dealing with 10 data points each. This test is also robust to small sample size, which was critical as our sample size was only 10 due to the computational space and run-time our models took up. We were unable to run the SVM and Random Forest model more than 10 times as this maxed out our RAM and Project Time on R Studio Cloud. The main assumption of a Wilcoxon Rank Sum Test is having two indpendent samples, which was satisfied.

Results: After conducting the Wilcoxon Rank-Sum Test for the Tuned Radial SVM (feature selected) and Tuned Random Forest (original data) models, we noted some important results. Our statistical testing included a **Z-value of -2.8031** and a **P-value of .00512**. Because our p-value was smaller than our significance level of 0.05, we reject the null hypothesis. The results are statistically significant at p<0.05.

**This means that the difference between the SVM model and the Random Forest model in regards to its F1 score is not due to chance alone.**
...
6. <https://medium.com/analytics-vidhya/machine-learning-decision-trees-and-random-forest-classifiers-81422887a544>
