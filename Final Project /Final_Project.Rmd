...
**General Question**: Will the SVM Model or the Random Forest Model Predict the Likelihood of a Person Getting the H1N1 Vaccine Better?
...
![Reference 3](/cloud/project/Photos/f1.png)
...
```{r, include=FALSE}
df_features <- read.csv("/cloud/project/Datasets/training_set_features.csv")
df_label <- read.csv("/cloud/project/Datasets/training_set_labels.csv")
...
df2<- read.csv("/cloud/project/Datasets/df2.csv")  
nrow(df)
```
...
coeff2dt(fitobject = cv.fit, s = "lambda.min") %>% head(15)
...
![SVM Optimization, Bias-Variance Tradeoff](/cloud/project/Photos/SVM_Optimization.jpg)
...
![Cost Savings Assessment in Potential Use Case](/cloud/project/Photos/Cost_Assessment_4002_Project1.jpg)
